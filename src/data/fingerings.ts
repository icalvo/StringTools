...
export function noteNumber(noteName: string): number|string {
    ...
    let octave = parseInt(noteName.charAt(pointer));
...
function calculateStop(instrument: Instrument, stringIndex: number, noteNumber: number): Stop|null {
    ...
    if (stopIndex < 0) {
        console.debug(`${noteName(noteNumber)} is too low for ${instrument.strings[stringIndex].name} string`);
        return null;
    }

    if (stopIndex > instrument.stops) {
        console.debug(`${noteName(noteNumber)} is too high for ${instrument.strings[stringIndex].name} string`);
        return null;
    }
...
function stops(instrument: Instrument, notes: number[]): Stop[][] {
    const result: (Stop)[][] = notes.map(note =>
        instrument.strings
            .map((_s, i) => calculateStop(instrument, i, note))
            .filter(f => f !== null)
    );
...
function cross(addValidation:Function, l1:Stop[][], l2:Stop[]): Stop[][] {
    ...
    return l1.flatMap(i1 =>
        l2.filter(i2 => addValidation(i1, i2))
            .map(i2 => i1.concat([i2])));
}
...
function stretchHardness(instrument: Instrument, stop1: Stop, stretch: number, stop2: Stop): number {
...
function fingeringHardness(instrument: Instrument, fingering: Stop[]): number {
    const sortedFingering = fingering
        .sort((s1, s2) => s1.stringIndex - s2.stringIndex)
...
export function calculateFingerings(instrument: Instrument, notes: number[], validations: Function[]): Stop[][] {
