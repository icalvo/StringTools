      ...
      <div class="flex-initial basis-1/2 p-4 text-wrap overflow-y-auto border-r-2 border-gray-300">
        ...
        <div>
          <div>
            ...
            <input id="stops" type="number" :value="instrument.stops" class="flex-1 border-2 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6" @input="event => instrumentsStore.changeStops(selectedInstrument, (event.target as HTMLInputElement).valueAsNumber)" />
          </div>
          <div class="pt-3">
          <CheckboxBase v-model="validateNoGaps" name="validateNoGaps" label="No gaps" class="pr-3" />
          <CheckboxBase v-model="validatePossibleStretch" name="validatePossibleStretch" label="Discard impossible stretches" class="pr-3" />
          <CheckboxBase v-model="includeNaturalHarmonics" name="includeNaturalHarmonics" label="Natural harmonics" />
          </div>
        </div>
