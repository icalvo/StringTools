...
export function abcnote(noteNumber: number): string {
  const noteIndex = noteNumber % 12;
  let noteName =
    ["c", "c", "d", "d", "e", "f", "f", "g", "g", "a", "a", "b"][noteIndex] ??
    "";
  const alteration =
    ["", "^", "", "^", "", "", "^", "", "^", "", "^", ""][noteIndex] ?? "";
...
function* stopsForString<TString extends InstrumentString>(
  ...
  console.debug(
    `Calculating stop for note ${noteNumber} on string ${stringIndex}`,
  );
  ...
  if (stopIndex < 0) {
    console.debug(
      `${noteName(noteNumber)} is too low for ${instrumentString.name} string`,
    );
    return;
  }
  ...
  if (stopIndex > instrument.stops) {
    console.debug(
      `${noteName(noteNumber)} is too high to stop for ${instrumentString.name} string`,
    );
  } else {
...
function cross(addValidation: Function, l1: Stop[][], l2: Stop[]): Stop[][] {
  ...
  return l1.flatMap((i1) =>
    l2.filter((i2) => addValidation(i1, i2)).map((i2) => i1.concat([i2])),
  );
}

function fingeringStretch<TString extends InstrumentString>(
  instrument: Instrument<TString>,
  stop1: Stop,
  stop2: Stop,
): number {
...
function fingeringStretches<TString extends InstrumentString>(
  instrument: Instrument<TString>,
  stops: Stop[],
): [Stop, number, Stop][] {
  return stops
    ...
    .map(([stop1, stop2]) => [
      stop1,
      fingeringStretch(instrument, stop1, stop2),
      stop2,
    ]);
}
...
function stretchesHardness<TString extends InstrumentString>(
  instrument: Instrument<TString>,
  stretches: [Stop, number, Stop][],
): number {
  return stretches
    .map(([stop1, stretch, stop2]) =>
      stretchHardness(instrument, stop1, stretch, stop2),
    )
    .reduce((a, b) => Math.max(a, b));
}

function fingeringHardness<TString extends InstrumentString>(
  instrument: Instrument<TString>,
  fingering: Stop[],
): number {
  const sortedFingering = fingering.sort(
    (s1, s2) => s1.stringIndex - s2.stringIndex,
  );

  return stretchesHardness(
    instrument,
    fingeringStretches(instrument, sortedFingering),
  );
}

export function hasNoGaps(
  _instrument: any,
  fingering: { stringIndex: number }[],
): boolean {
  return fingering
...
export function hasPossibleStretch<TString extends InstrumentString>(
  instrument: Instrument<TString>,
  fingering: Stop[],
): boolean {
...
export function calculateFingerings<TString extends InstrumentString>(
  ...
  const stopsByNote = stopsForInstrument(
    instrument,
    notes,
    includeNaturalHarmonics,
  );
  ...
  function stringIsNotRepeated(stopList: Stop[], stop: Stop): boolean {
    return stopList.every(
      (existingStop) => existingStop.stringIndex !== stop.stringIndex,
    );
  }

  function validation(fng: Stop[]): boolean {
    return validations
      .map((val) => val(instrument, fng))
      .every((result) => result);
  }
}

export function calculateFingeringSequence<TString extends InstrumentString>(
  instrument: Instrument<TString>,
  chords: number[][],
  validations: Function[],
  includeNaturalHarmonics: boolean = false,
