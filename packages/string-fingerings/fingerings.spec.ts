...
describe("hasNoGaps", () => {
  it("Returns expected results", () => {
    expect(hasNoGaps(undefined, [{ stringIndex: 3 }, { stringIndex: 4 }])).toBe(
      true,
    );
    expect(hasNoGaps(undefined, [{ stringIndex: 1 }, { stringIndex: 3 }])).toBe(
      false,
    );
    expect(
      hasNoGaps(undefined, [
...
describe("calculateFingerings", () => {
  ...
  it("works with quadruple stop C-major chord", () => {
    expect(
      calculateFingerings(violin, [60, 67, 72, 76], [], false),
    ).toStrictEqual([
      [
        {
          stringIndex: 0,
...
describe("calculateFingeringSequences", () => {
  it("works with semitone slide", () => {
    expect(
      calculateFingeringSequence(
        violin,
        [
